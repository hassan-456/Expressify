...
void main() {
  ...
  for (float i = 0.0; i < 1.0; i += 1.0 / NUM_LAYER) {
    ...
    float scale = mix(100.0 * uDensity, 5.0 * uDensity, depth);
 ...
 * @param {{ soft?: boolean, glowIntensity?: number, density?: number, twinkleIntensity?: number, rotationSpeed?: number, starSpeed?: number, saturation?: number, hueShift?: number }} [options]
 */
export function initGalaxy(containerId, options = {}) {
    ...
    const starSpeed = soft ? 0.32 : (options.starSpeed ?? 0.5);
    // const density = soft ? 0.42 : (options.density ?? 1);
    const density = soft ? 0.42 : (options.density ?? 1);

    const hueShift = options.hueShift ?? (soft ? 155 : 140);
    const disableAnimation = false;
    const speed = soft ? 0.65 : 1.0;
    const mouseInteraction = true;
    const glowIntensity = soft ? 0.07 : (options.glowIntensity ?? 0.3);
    const saturation = soft ? 0.22 : (options.saturation ?? 0.0);
    const mouseRepulsion = true;
    const repulsionStrength = soft ? 1.2 : 2;
    const twinkleIntensity = soft ? 0.02 : (options.twinkleIntensity ?? 0.3);
    const rotationSpeed = soft ? 0.04 : (options.rotationSpeed ?? 0.1);
